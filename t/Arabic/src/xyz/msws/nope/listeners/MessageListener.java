...
public class MessageListener
    extends AbstractModule implements PluginMessageListener {

  public MessageListener(NOPE plugin) { super(plugin); }
  ...
  public void onPluginMessageReceived(String channel, Player player,
    ...
    if (msg.startsWith("perm:")) {
      ...
      OfflinePlayer off = Bukkit.getOfflinePlayer(
          msg.split(" ")[0].substring("setvl:".length()));
      ...
      String reason =
          String.join(" ", (CharSequence[])ArrayUtils.subarray(
                               msg.split(" "), 2, msg.split(" ").length));
      plugin.getModule(Banwave.class)
          .addPlayer(off.getUniqueId(),
                     wave.new BanwaveInfo(off.getUniqueId(), decoy,
                                          Long.parseLong(msg.split(" ")[1]),
                                          reason));
    } else if (msg.startsWith("removebanwave:")) {
      OfflinePlayer off = Bukkit.getOfflinePlayer(
          msg.split(" ")[0].substring("removebanwave:".length()));
