  ...
  static {
    ...
    if (System.getProperty("bstats.relocatecheck") == null ||
      ...
      final String defaultPackage =
          new String(new byte[] {'o', 'r', 'g', '.', 'b', 's', 't', 'a', 't',
                                 's', '.', 'b', 'u', 'k', 'k', 'i', 't'});
  ...
  private void startSubmitting() {
    ...
    timer.scheduleAtFixedRate(new TimerTask() {
      ...
      public void run() {
        ...
        // have to use the Bukkit scheduler Don't be afraid! The connection to
        // the bStats server is still async, only the stats collection is sync
        // ;)
        Bukkit.getScheduler().runTask(plugin, () -> submitData());
      }
    }, 1000 * 60 * 5, 1000 * 60 * 30);
    // Submit the data every 30 minutes, first time after 5 minutes to give
    // other plugins enough time to start WARNING: Changing the frequency has no
    // effect but your plugin WILL be blocked/deleted! WARNING: Just don't do
    // it!
  }
              ...
              try {
                ...
                if (logFailedRequests) {
                  this.plugin.getLogger().log(
                      Level.SEVERE, "Encountered unexpected exception", e);
                }
              }
            }
