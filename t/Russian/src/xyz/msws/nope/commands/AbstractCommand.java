...
public abstract class AbstractCommand расширяет AbstractModule реализует
    CommandExecutor,
    TabCompleter {

  защищённый список<Subcommand> cmds = new ArrayList<>();

  public AbstractCommand(NOPE plugin) { супер(плагин); }
  ...
  публичный список<Subcommand> getSubCommands() { возвращать команд; }
  ...
  public boolean onCommand(CommandSender sender, Command command, String label,
    ...
    for (Subcommand c : cmds) {
      if (c.getName().equalsIgnoreCase(cmd) ||
          c.getAliases().contains(cmd.toLowerCase())) {
        ...
        MSG.tell(sender, "&4" + label + " > &cПравильное использование для " +
                             c.getName());
