...
public enum CommandResult {
  ...
  публичная строка getMessage() {
    сменить(это) {
    НЕВЕРНОЙ_ARGUMENT:
      return MSG.getString("Command.InvalidArgument",
                           "&cAn invalid argument was provided.");
      случай MISSING_ARGUMENT
          : return MSG.getString("Command.MissingArgument",
                                 "&cYou are missing an argument.");
      регистр NO_PERMISSION
          : return MSG.getString(
                "Command.NoPermission",
                "&4&l[&c&lNOPE&4&l] &cВы не имеете &a%perm% &cpermission.");
      регистр PLAYER_ТОЛЬКО возвращать MSG.getString(
          "Command.PlayerOnly",
          "&cВы должны указать игрока, чтобы запустить эту команду как консоль.");
      регистр PLAYER_REQUIRED
          : возвращать MSG.getString(
                "Command.SpecifyPlayer",
                "&cYou должен указать игрока в качестве аргумента.");
      регистр УСПЕШС : вернуть "";
      по умолчанию : разрыва;
    }
